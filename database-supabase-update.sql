-- =============================================================================
-- DATABASE UPDATE SCRIPT (voor bestaand Supabase Cloud project)
-- Project: Mijn Aarde - Ledenadministratie
-- Generated: 2026-02-16
--
-- Dit script brengt een bestaand Supabase-project bij naar het huidige schema.
-- Het gebruikt IF NOT EXISTS / IF EXISTS waar mogelijk zodat het veilig
-- meerdere keren uitgevoerd kan worden (idempotent).
--
-- INSTRUCTIES:
-- 1. Ga naar je Supabase dashboard > SQL Editor
-- 2. Plak dit script en voer het uit
-- 3. Controleer of alle tabellen en policies correct zijn aangemaakt
-- =============================================================================

-- =============================================================================
-- 1. ENUM (aanmaken indien niet bestaat)
-- =============================================================================
...
-- =============================================================================
-- 2. TABLES (aanmaken indien niet bestaan)
-- =============================================================================
...
-- Kolommen toevoegen die mogelijk ontbreken op bestaande members tabel
DO $$ BEGIN
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS date_of_birth DATE;
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS is_active_member BOOLEAN DEFAULT true;
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS is_ambassador BOOLEAN DEFAULT false;
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS is_donor BOOLEAN DEFAULT false;
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS is_council_member BOOLEAN DEFAULT false;
    ALTER TABLE public.members ADD COLUMN IF NOT EXISTS password_change_required BOOLEAN NOT NULL DEFAULT false;
...
-- Kolom toevoegen die mogelijk ontbreekt
DO $$ BEGIN
    ALTER TABLE public.expenses ADD COLUMN IF NOT EXISTS category TEXT DEFAULT 'overig';
END $$;
...
CREATE TABLE IF NOT EXISTS public.annual_report_inventory (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    fiscal_year INTEGER NOT NULL,
    category TEXT NOT NULL,
    type TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.mailing_assets (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
SELECT id, name, address, postal_code, city, country, website, is_supplier, created_at, updated_at
FROM public.companies;

-- =============================================================================
-- 4. FUNCTIONS (CREATE OR REPLACE = altijd veilig)
-- =============================================================================
...
-- =============================================================================
-- 5. TRIGGERS (verwijder eerst bestaande, dan opnieuw aanmaken)
-- =============================================================================
...
DROP TRIGGER IF EXISTS update_annual_report_inventory_updated_at ON public.annual_report_inventory;
...
DROP TRIGGER IF EXISTS update_mailing_templates_updated_at ON public.mailing_templates;
...
DROP TRIGGER IF EXISTS update_mailings_updated_at ON public.mailings;
...
DROP TRIGGER IF EXISTS update_mailing_assets_updated_at ON public.mailing_assets;
...
-- Verwijdert eerst alle bestaande policies per tabel, dan opnieuw aanmaken.
-- Dit maakt het script idempotent.
-- =============================================================================

-- Helper: verwijder alle policies op een tabel
DO $$ 
DECLARE
    _tbl TEXT;
    _pol RECORD;
BEGIN
    FOR _tbl IN SELECT unnest(ARRAY[
        'companies','members','user_roles','tags','member_tags',
        'events','event_registrations','income','expenses','donations',
        'contributions','invoices','invoice_items','budget',
        'annual_report_inventory','mailing_templates','mailings','mailing_assets'
    ]) LOOP
        FOR _pol IN 
...
CREATE POLICY "Members can update own profile fields" ON public.members
...
CREATE POLICY "Admins can manage mailing assets" ON public.mailing_assets
    FOR ALL USING (has_role(auth.uid(), 'admin'));
CREATE POLICY "Public can read logo assets" ON public.mailing_assets
    FOR SELECT USING (type = 'logo');
...
CREATE INDEX IF NOT EXISTS idx_member_tags_tag_id ON public.member_tags(tag_id);
CREATE INDEX IF NOT EXISTS idx_tags_name ON public.tags(name);
CREATE INDEX IF NOT EXISTS idx_inventory_fiscal_year ON public.annual_report_inventory(fiscal_year);
CREATE INDEX IF NOT EXISTS idx_budget_fiscal_year ON public.budget(fiscal_year);
CREATE INDEX IF NOT EXISTS idx_invoice_items_invoice_id ON public.invoice_items(invoice_id);
CREATE INDEX IF NOT EXISTS idx_invoices_company_id ON public.invoices(company_id);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_year ON public.invoices(invoice_year);
...
CREATE INDEX IF NOT EXISTS idx_contributions_year ON public.contributions(contribution_year);
...
-- Verwijder bestaande storage policies en maak opnieuw aan
DO $$
DECLARE
    _pol RECORD;
BEGIN
    FOR _pol IN 
        ...
        AND policyname IN (
            'Admins can manage profile photos',
            'Members can view their own profile photo',
            'Anyone can view mailing assets',
            'Admins can manage mailing assets storage',
...
CREATE POLICY "Admins can manage profile photos" ON storage.objects
    FOR ALL TO authenticated USING (bucket_id = 'profile-photos' AND has_role(auth.uid(), 'admin'));

CREATE POLICY "Members can view their own profile photo" ON storage.objects
    FOR SELECT TO authenticated USING (bucket_id = 'profile-photos');

CREATE POLICY "Anyone can view mailing assets" ON storage.objects
    FOR SELECT USING (bucket_id = 'mailing-assets');

CREATE POLICY "Admins can manage mailing assets storage" ON storage.objects
    FOR ALL TO authenticated USING (bucket_id = 'mailing-assets' AND has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can manage receipts" ON storage.objects
    FOR ALL TO authenticated USING (bucket_id = 'receipts' AND has_role(auth.uid(), 'admin'));

CREATE POLICY "Members can view their own receipts" ON storage.objects
    FOR SELECT TO authenticated USING (bucket_id = 'receipts');

-- =============================================================================
-- 9. CONFIGURATIE NA UITVOERING
-- =============================================================================
-- 
-- Configureer de volgende secrets (Edge Functions > Secrets):
--   SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD
--   SMTP_FROM_EMAIL, SMTP_FROM_NAME
--   MOLLIE_API_KEY, RESEND_API_KEY, MAILERSEND_API_KEY
--
